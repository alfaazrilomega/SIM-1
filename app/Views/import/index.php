...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ===== MAIN ===== */
    main { flex: 1; padding: 2.5rem 1.5rem; max-width: 980px; margin: 0 auto; width: 100%; }
    .page-header { margin-bottom: 2rem; }
    .page-header h1 {
      font-family: var(--font-head); font-size: 1.8rem; font-weight: 800; letter-spacing: -.5px;
    ...
    .card {
      ...
      border-radius: var(--r); padding: 1.75rem; transition: border-color .25s;
    }
    .card:hover { border-color: rgba(79,142,247,.22); }
    .card-label {
      font-size: .7rem; text-transform: uppercase; letter-spacing: 1.2px;
      color: var(--muted); margin-bottom: 1.2rem; display: flex; align-items: center; gap: .4rem;
    }

    #drop-zone {
      border: 2px dashed rgba(79,142,247,.28); border-radius: var(--r);
      padding: 2.75rem 2rem; text-align: center; cursor: pointer;
      background: rgba(79,142,247,.025); transition: all .25s; user-select: none;
    }
    #drop-zone:focus { outline: 2px solid var(--accent); outline-offset: 3px; }
    #drop-zone.drag-over { border-color: var(--accent); background: rgba(79,142,247,.07); transform: scale(1.008); }
    #drop-zone.has-file  { border-color: var(--success); background: rgba(34,197,94,.04); }
    .dz-icon { font-size: 2.8rem; color: var(--accent); display: block; margin-bottom: .7rem; transition: transform .3s; }
    ...
    #file-strip {
      ...
      background: rgba(34,197,94,.07); border: 1px solid rgba(34,197,94,.2); border-radius: 10px;
    }
    ...
    #btn-import {
      width: 100%; margin-top: 1.4rem; padding: .95rem 1.5rem;
      background: linear-gradient(135deg, var(--accent), var(--accent2));
    ...
    #btn-import:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(79,142,247,.4); }
    ...
    .prog-fill  { height: 100%; border-radius: 99px; background: linear-gradient(90deg, var(--accent), var(--accent2)); transition: width .4s ease; width: 0%; }
    ...
    .chip { background: var(--bg-card2); border: 1px solid var(--border); border-radius: 12px; padding: 1rem; text-align: center; }
    ...
    .table-card-head { padding: 1rem 1.4rem; border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: .5rem; }
    ...
    tbody td { padding: .65rem 1rem; border-bottom: 1px solid rgba(255,255,255,.04); white-space: nowrap; }
    tbody tr:last-child td { border-bottom: none; }
    tbody tr:hover td { background: rgba(79,142,247,.04); }

    .badge { display: inline-flex; align-items: center; gap: .3rem; padding: .22rem .6rem; border-radius: 20px; font-size: .7rem; font-weight: 700; }
...
<div class="page">
  ...
  <header>
    ...
    <div class="header-nav">
      ...
      <a href="<?= base_url('/import') ?>"        class="btn-nav active"><i class="bi bi-cloud-arrow-up"></i> Import</a>
    </div>
  </header>
    ...
    <div class="grid">
      <div>
        <div class="card">
          ...
          <div id="drop-zone" tabindex="0" role="button" aria-label="Klik atau seret file Excel .xlsx ke area ini">
        ...
        <div id="results-wrap">
          ...
          <div class="table-card">
            ...
            <div class="table-wrap">
              <table>
                <thead>
                  <tr><th>Order ID</th><th>Status</th><th>Produk (Sample)</th><th>Items</th><th>Total (Rp)</th><th>Aksi</th></tr>
                </thead>
      ...
      <div>
        <div class="card" style="margin-bottom:1rem">
          ...
          <ul class="info-list">
            <li><i class="bi bi-arrow-repeat ii" style="color:var(--warning)"></i><div class="it"><strong>Upsert by Order ID</strong><span>Sudah ada → UPDATE. Baru → INSERT. Tanpa duplikat.</span></div></li>
            <li><i class="bi bi-lock-fill ii" style="color:var(--success)"></i><div class="it"><strong>Status Penarikan CEO Aman</strong><span><code>status_penarikan</code> TIDAK direset saat re-import.</span></div></li>
            <li><i class="bi bi-funnel-fill ii" style="color:var(--accent)"></i><div class="it"><strong>Laporan filter "Selesai"</strong><span>View laporan hanya tampilkan pesanan berstatus <em>Selesai</em>.</span></div></li>
            <li><i class="bi bi-tags-fill ii" style="color:#c084fc"></i><div class="it"><strong>Kombinasi Produk + Variasi</strong><span>Variasi "Default" → cek tabel <code>product_mapping</code>. Non-Default → nama + variasi.</span></div></li>
            <li><i class="bi bi-recycle ii" style="color:var(--warning)"></i><div class="it"><strong>Detail Rebuild Otomatis</strong><span>Item lama dihapus & diganti versi terbaru setiap re-import.</span></div></li>
          </ul>
        </div>
  ...
  <footer>SIM Import Tool &nbsp;·&nbsp; CodeIgniter 4 &nbsp;·&nbsp; Platform: <code>TikTok OrderSKUList</code> &nbsp;·&nbsp; DB: <code>sim_orders</code></footer>
</div>
...
function setFile(f) {
  if (!f) return;
  if (!f.name.toLowerCase().endsWith('.xlsx')) { alert('❌ Hanya file .xlsx yang diizinkan.\n\nFile kamu: ' + f.name); return; }
  activeFile = f;
...
const startProgress = () => {
  ...
  progTimer = setInterval(() => {
    ...
    progLabel.textContent = pct < 25 ? 'Membaca file Excel…' : pct < 55 ? 'Memproses baris data…' : 'Mengupdate database…';
  }, 320);
};
const endProgress = (ok = true) => {
  ...
  setTimeout(() => { progWrap.style.display = 'none'; progFill.style.width = '0%'; progFill.style.background = ''; progLabel.classList.add('pulsing'); }, 2000);
};
...
function renderResults(d) {
  ...
  if (d.errors && d.errors.length) {
    ...
    errBox.innerHTML = '<i class="bi bi-exclamation-triangle"></i> <strong>Error:</strong> ' + d.errors.map(e => '<div>' + esc(e) + '</div>').join('');
  ...
  (d.preview||[]).forEach(row => {
    ...
    prevTbody.insertAdjacentHTML('beforeend', `
      <tr>
        ...
        <td><span class="badge ${isNew ? 'badge-new':'badge-updated'}"><i class="bi ${isNew ? 'bi-plus-circle':'bi-arrow-repeat'}"></i>${row.action}</span></td>
      </tr>`);
  });
...
btnImport.addEventListener('click', async () => {
  ...
  try {
    const resp = await fetch('<?= base_url('/import/process') ?>', { method:'POST', body:fd, headers:{ 'X-Requested-With':'XMLHttpRequest' } });
    ...
    if (data.success) {
      ...
      endProgress(false);
      errBox.style.display = 'block';
      errBox.innerHTML = '<i class="bi bi-exclamation-triangle"></i> <strong>Error:</strong> ' + esc(data.error || 'Terjadi kesalahan.') + (data.hint ? '<br><small style="opacity:.8">' + esc(data.hint) + '</small>' : '');
      statRow.innerHTML = ''; prevTbody.innerHTML = '';
      resultsWrap.style.display = 'block';
    }
  } catch (err) { endProgress(false); alert('Request gagal: ' + err.message); }
  finally { btnImport.disabled = false; btnIcon.className = 'bi bi-database-up'; btnText.textContent = 'Import ke Database'; }
});
</script>
</body>
</html>